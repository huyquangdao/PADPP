    ...
    def execute(self):
        ...
        if self.model_config.test_phase:
            ...
            if self.model_config.run_online_eval:
                ...
                if self.model_config.ablation != '':
                    logger.info(
                        f"Performing ablation study: {self.model_config.ablation} .....")
                    if 'rl' in self.model_config.ablation:
                        self.load_pretrained_model(is_rl=False)
                else:
                    logger.info(
                        "Loading the supervised RL fine-tuning model .....")
        ...
        # online evaluation
            ...
            if self.model_config.run_sft:
                logger.info(
                    "Performing supervised fine-tuning on the background dataset ...")
                self.run_sft()
            ...
            if self.model_config.run_rlt:
                ...
                logger.info(
                    "Performing reinforcement learning fine-tuning ....")
                # then we fine tune the model with reinforcement learning
                self.run_rlt()
...
class PADPPPipelineForRecommendation(PADPPPipeline):
    ...
    def run_online_test(self, target_items=None, simulators=None):
        ...
        if target_items is not None:
            ...
            if self.model_config.num_test_cases:
                ...
                test_target_items = random.sample(
                    test_target_items, self.model_config.num_test_cases)
        ...
        if len(test_simulators) > len(test_target_items):
            test_simulators = random.sample(
                test_simulators, len(test_target_items))
...
class PADPPPipelineForNegotiation(PADPPPipeline):
...
class PADPPPipelineForEmotionalSupport(PADPPPipeline):
