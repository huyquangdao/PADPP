...
#
# from modpl_new.config import PADPPConfig
...
if __name__ == '__main__':
    ...
    game_config_file, game_config_class, game_class, game_simulator_class = get_scenario_by_name(
        args['scenario'])
    ...
    dataset_config_classes_and_config_paths = get_datasets_by_names(
    ...
    generation_packges = get_text_generation_model_by_name(
    ...
    for data_config_path, dataset_class, dataset_scenario_config_class in dataset_config_classes_and_config_paths:
        ...
        # load the user simulators from file
            ...
            dev_user_simulators = load_user_simulators(
                simulator_file_path=dataset_config.save_dev_simulator_path)
            test_user_simulators = load_user_simulators(
                simulator_file_path=dataset_config.save_test_simulator_path)
        ...
        for (
            ...
            # create the game
            game = game_class(game_config=game_config,
            ...
            if args['ablation'] is not None:
                # if isinstance(model_config, PADPPConfig) or isinstance(model_config, EnvelopeConfig) or isinstance(model_config, MinDistPADPPConfig):
                if isinstance(model_config, PADPPConfig):
                    model_config.set_params(
                        {
            ...
            # we only consider the MODPL model
            # if isinstance(model_config, PADPPConfig) or isinstance(model_config, PPDPPConfig) or isinstance(model_config, TRIPConfig) \
            ...
            if isinstance(model_config, PADPPConfig):
                ...
                # if args['objective_weight'] is not None and (isinstance(model_config, PADPPConfig) or isinstance(model_config, EnvelopeConfig)) or isinstance(model_config, MinDistPADPPConfig):
                if args['objective_weight'] is not None and isinstance(model_config, PADPPConfig):

                    print(args['objective_weight'])
                    # convert objective weights to float format
                    objective_weight=[float(x)
                                        for x in args['objective_weight']]
            ...
            for gen_name, generation_package in list(zip(args['gen_models'].split(','), generation_packges)):
                ...
                if gen_name.strip() == BART_GENERATION:
                    ...
                    generation_params=load_config_from_yaml_file(
                        generation_config_path)
                    generation_config=generation_config_class(
                        generation_params)
                    ...
                    generation_model=generation_model_class(
                        generation_config)
                    generation_model_name=str(
                        generation_model.__class__.__name__)
